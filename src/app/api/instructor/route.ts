...
export async function GET() {
  try {
    const res = await fetch('https://api.ayoub.htufolio.com:8080/instructor?departmentName=COMPUTER_SCIENCE', {
      cache: 'no-store',
    });
...
export async function POST(request: Request) {
    try {
      ...
      const res = await fetch('https://api.ayoub.htufolio.com:8080/instructor', {
        method: 'POST',
        headers: {
